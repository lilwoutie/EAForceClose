...
The admin acces is purely because there are EA backround processes that keep running on the SYSTEM and not on the user, so the script closes ALL EA processes when you press the X in the app of EA. Feel free to remove the admin rights if you feel uncomfortable running this script with admin rights but then the background processes of EA on the SYSTEM level will keep running. But you can ofcourse just read and interpret the script and see nothing sketchy is going on, AI can help you with this.
